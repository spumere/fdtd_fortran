program fdtd2d
    ...
    real, allocatable::ex_res(:,:,:), ey_res(:,:,:), hz_res(:,:,:), t_res(:)
    ...
    allocate(ex_res(ie, jb, nmax), ey_res(ib, je, nmax), hz_res(ie, je, nmax), t_res(nmax))
    ...
    do n=1, nmax
        ...
        ! Сохранение полей для визуализации и сравнения с аналитическим решением
        do i=1, ie
            do j=1, jb
                ex_res(i,j,n) = ex(i,j)
            end do
    ...
    open(newunit=results, file='fields2d.bin', &
        form='unformatted', access='stream', status='replace')
    write(results) int(ie, 4), int(jb, 4), int(ib, 4), int(je, 4), int(nmax, 4)
    write(results) real(dx, 4), real(dt, 4)

    write(results) real(t_res(1:nmax), 4)
    do n = 1, nmax
        write(results) real(ex_res(:,:,n), 4)
        write(results) real(ey_res(:,:,n), 4)
        write(results) real(hz_res(:,:,n), 4) 
    end do

    close(results)
    call execute_command_line('python plot_2d.py')

    deallocate(source, ca, cb, da, db, t_res, ex_res, ey_res, hz_res)
end program fdtd2d