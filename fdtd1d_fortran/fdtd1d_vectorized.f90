program fdtd1d_vectorized
    ...
    ez = 0.0
    hy = 0.0
    open(newunit=u_ez, file='vectorized_results/ez_vectorized.dat', status='replace')
    open(newunit=u_hy, file='vectorized_results/hy.dat', status='replace')
    ...
    close(u_ez)
    close(u_hy)
    call execute_command_line('gnuplot -persist results\vectorized_plot.gp')

end program fdtd1d_vectorized